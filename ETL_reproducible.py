...
def load(fact_sales, fact_inventory, replenishment_plan):
    ...
    fact_sales.to_csv('sampler/fact_sales_store_sku_daily.csv', index=False)
    fact_inventory.to_csv('sampler/fact_inventory_store_sku_daily.csv', index=False)
    replenishment_plan.to_csv('sampler/replenishment_inputs_store_sku.csv', index=False)
